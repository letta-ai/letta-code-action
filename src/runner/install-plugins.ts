...
/**
 * Executes a Letta Code CLI command with proper error handling
 * @param lettaExecutable - Path to the Letta executable
 ...
 */
async function executeLettaCommand(
  lettaExecutable: string,
  args: string[],
  errorContext: string,
): Promise<void> {
  return new Promise((resolve, reject) => {
    const childProcess: ChildProcess = spawn(lettaExecutable, args, {
      stdio: "inherit",
    });
...
/**
 * Installs a single Letta Code plugin
...
/**
 * Adds a Letta Code plugin marketplace
 * @param lettaExecutable - Path to the Letta executable
...
/**
 * Installs Letta Code plugins from a newline-separated list
 ...
 * @param lettaExecutable - Path to the Letta executable (defaults to "letta")
...
export async function installPlugins(
  ...
  const resolvedExecutable = lettaExecutable || "letta";
