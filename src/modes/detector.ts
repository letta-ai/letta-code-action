...
export function detectMode(context: GitHubContext): AutoDetectedMode {
  ...
  if (isEntityContext(context)) {
    if (
      ...
      // Default to tag mode if @letta-code mention found
  ...
  if (isEntityContext(context) && isIssuesEvent(context)) {
    ...
    // Check for @letta-code mentions or labels/assignees
    if (checkContainsTrigger(context)) {
      return "tag";
    }
  }
...
export function getModeDescription(mode: AutoDetectedMode): string {
  switch (mode) {
    case "tag":
      return "Interactive mode triggered by @letta-code mentions";
    case "agent":
