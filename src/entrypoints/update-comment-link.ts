...
async function run() {
  try {
    const commentId = parseInt(process.env.LETTA_COMMENT_ID!);
    const githubToken = process.env.GITHUB_TOKEN!;
    const lettaBranch = process.env.LETTA_BRANCH;
    ...
    if (!prepareSuccess && prepareError) {
      ...
      try {
        ...
        // Check if the Letta action failed
        const lettaSuccess = process.env.LETTA_SUCCESS !== "false";
        actionFailed = !lettaSuccess;
      } catch (error) {
        console.error("Error reading output file:", error);
        // If we can't read the file, check for any failure markers
        actionFailed = process.env.LETTA_SUCCESS === "false";
      }
    }
    ...
    const commentInput: CommentUpdateInput = {
      ...
      branchLink,
      prLink,
      branchName: shouldDeleteBranch || !branchLink ? undefined : lettaBranch,
      triggerUsername,
      errorDetails,
      agentId,
      model,
    ...
    try {
      await updateLettaComment(octokit.rest, {
        owner,
        repo,
        commentId,
        body: updatedBody,
