...
/**
 * Prepare the Letta action by checking trigger conditions, verifying human actor,
...
async function run() {
  try {
    ...
    // Letta uses Skills instead of MCP - the github-action skill teaches the agent to use gh CLI

    // Expose the GitHub token (Letta App token) as an output
    core.setOutput("github_token", githubToken);
    ...
    if (mode.getSystemPrompt) {
      const modeContext = mode.prepareContext(context, {
        ...
        lettaBranch: result.branchInfo.lettaBranch,
      });
