...
export function buildAllowedToolsString(
  ...
  _includeActionsTools: boolean = false,
...
export function prepareContext(
  ...
  const triggerPhrase = context.inputs.triggerPhrase || "@letta-code";
  ...
  switch (eventName) {
    ...
    case "issue_comment":
      ...
      if (isPR) {
        ...
        throw new Error("LETTA_BRANCH is required for issue_comment event");
      } else if (!baseBranch) {
    ...
    case "issues":
      ...
      if (!lettaBranch) {
        throw new Error("LETTA_BRANCH is required for issues event");
      }
...
function getCommitInstructions(
  ...
  // Letta uses git commands directly
  if (eventData.isPR && !eventData.lettaBranch) {
    return `
    ...
    const branchName = eventData.lettaBranch || eventData.baseBranch;
    return `
      - You are already on the correct branch (${eventData.lettaBranch || "the PR branch"}). Do not create a new branch.
      - Use git commands via the Bash tool to commit and push your changes:
        - Stage files: Bash(git add <files>)
        - Commit with a descriptive message: Bash(git commit -m "<message>")
        ${
...
<letta_comment_id>${context.lettaCommentId}</letta_comment_id>
...
${`<comment_update_info>
IMPORTANT: Load the github-action skill to learn how to update your tracking comment.

Update your comment using gh CLI:
\`\`\`bash
gh api /repos/${context.repository}/issues/comments/${context.lettaCommentId} -X PATCH -f body="Your updated content here"
\`\`\`

Update as you work to show progress to the user.
</comment_update_info>`}
...
1. Create a Todo List:
   ...
   - Update your tracking comment via gh api with each task completion.
...
4. Execute Actions:
   ...
   A. For Answering Questions and Code Reviews:
      - If asked to "review" code, provide thorough code review feedback:
        ...
        - Reference specific code sections with file paths and line numbers${eventData.isPR ? `\n      - AFTER reading files and analyzing code, you MUST update your tracking comment via gh api to post your review` : ""}
...
- Never create new comments. Only update your existing tracking comment via gh api.
