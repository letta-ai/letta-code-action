...
describe("Agent Mode", () => {
  ...
  test("prepare method passes through letta_args", async () => {
    ...
    // Set LETTA_ARGS environment variable
    process.env.LETTA_ARGS = "--model sonnet-4 --max-turns 10";
    ...
    // Verify letta_args includes user args (no MCP config in agent mode without allowed tools)
    const callArgs = setOutputSpy.mock.calls[0];
    expect(callArgs[0]).toBe("letta_args");
    expect(callArgs[1]).toBe("--model sonnet-4 --max-turns 10");
