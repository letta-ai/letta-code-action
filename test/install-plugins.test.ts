...
import { installPlugins } from "../src/runner/install-plugins";
...
describe("installPlugins", () => {
  ...
  test("should use custom letta executable path when provided", async () => {
    const spy = createMockSpawn();
    await installPlugins(undefined, "test-plugin", "/custom/path/to/letta");

    expect(spy).toHaveBeenCalledTimes(1);
    // Only call: install plugin (no marketplace without explicit marketplace input)
    expect(spy).toHaveBeenNthCalledWith(
      1,
      "/custom/path/to/letta",
      ["plugin", "install", "test-plugin"],
      { stdio: "inherit" },
    );
  });
  ...
  test("should install plugins with custom executable and multiple plugins", async () => {
    ...
    await installPlugins(
      undefined,
      "plugin-a\nplugin-b",
      "/usr/local/bin/letta-code-custom",
    );
