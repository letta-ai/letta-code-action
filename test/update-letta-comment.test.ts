...
import {
  updateLettaComment,
  type UpdateLettaCommentParams,
} from "../src/github/operations/comments/update-letta-comment";

describe("updateLettaComment", () => {
  ...
  test("should update issue comment successfully", async () => {
    ...
    const params: UpdateLettaCommentParams = {
      owner: "testowner",
      repo: "testrepo",
      commentId: 123456,
      body: "Updated comment",
    ...
    const result = await updateLettaComment(mockOctokit, params);
  ...
  test("should propagate error when PR review comment update fails with non-404 error", async () => {
    ...
    await expect(updateLettaComment(mockOctokit, params)).rejects.toEqual(
      mockError,
    );
