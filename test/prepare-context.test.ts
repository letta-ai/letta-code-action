    ...
    describe("on issue", () => {
      beforeEach(() => {
        process.env = {
          ...BASE_ENV,
          BASE_BRANCH: "main",
          LETTA_BRANCH: "letta/issue-67890-20240101-1200",
        };
      });
      ...
      test("should parse issue_comment event correctly", () => {
        const result = prepareContext(
          ...
          "12345",
          "main",
          "letta/issue-67890-20240101-1200",
        );

        expect(result.repository).toBe("test-owner/test-repo");
        expect(result.lettaCommentId).toBe("12345");
        expect(result.triggerPhrase).toBe("@letta-code");
        ...
        if (
          ...
          expect(result.eventData.lettaBranch).toBe(
            "letta/issue-67890-20240101-1200",
          );
          expect(result.eventData.baseBranch).toBe("main");
          expect(result.eventData.commentBody).toBe(
            "@letta-code can you help explain how to configure the logging system?",
          );
        }
      });

      test("should throw error when LETTA_BRANCH is missing", () => {
        expect(() =>
          prepareContext(mockIssueCommentContext, "12345", "main"),
        ).toThrow("LETTA_BRANCH is required for issue_comment event");
      });
    ...
    describe("on PR", () => {
      test("should parse PR issue_comment event correctly", () => {
        ...
        if (
          ...
          expect(result.eventData.commentBody).toBe(
            "/letta-code please review the changes and ensure we're not introducing any new memory issues",
          );
        }
      });
    });
  ...
  describe("pull_request_review event", () => {
    test("should parse pull_request_review event correctly", () => {
      ...
      if (result.eventData.eventName === "pull_request_review") {
        ...
        expect(result.eventData.commentBody).toBe(
          "@letta-code can you check if the error handling is comprehensive enough in this PR?",
        );
      }
    });
  });
  ...
  describe("pull_request_review_comment event", () => {
    test("should parse pull_request_review_comment event correctly", () => {
      ...
      if (result.eventData.eventName === "pull_request_review_comment") {
        ...
        expect(result.eventData.commentBody).toBe(
          "/letta-code is this the most efficient way to implement this algorithm?",
        );
      }
    });
  });
  ...
  describe("issues event", () => {
    beforeEach(() => {
      process.env = {
        ...BASE_ENV,
        BASE_BRANCH: "main",
        LETTA_BRANCH: "letta/issue-42-20240101-1200",
      };
    });
    ...
    test("should parse issue assigned event correctly", () => {
      ...
      if (
        ...
        expect(result.eventData.assigneeTrigger).toBe("@letta-bot");
      }
    });

    test("should throw error when LETTA_BRANCH is missing for issues", () => {
      expect(() =>
        prepareContext(mockIssueOpenedContext, "12345", "main"),
      ).toThrow("LETTA_BRANCH is required for issues event");
    });
